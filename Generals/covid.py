    ...
    async def covid(self, ctx, *, countryName = None):
        try:
            if countryName is None:
                embed=discord.Embed(title="to use this simply mention a country or world", colour=0xff0000, timestamp=ctx.message.created_at)
            ...
            await ctx.send(embed=embed3, delete_after = 10)
            await ctx.message.delete()
