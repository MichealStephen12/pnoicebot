...
class GiveAwayCommand(commands.Cog):
    ...
    async def giveaway(self, ctx):
        ...
        try:
            ...
            bro = int(msg.content)
            await ctx.send(f"Ok there will be {bro} winners\nHow many seconds should it last? *Check this link to convert seconds to minutes/hours:* https://bit.ly/37BgQ0L, **AND PUT NUMBER ONLY DONT DO 1m instead put 60 for 1 minute**")
            msg = await self.bot.wait_for('message', timeout=25.0, check=is_me)
            second = msg.content
            ...
            giveawayembed.set_footer(text="by DaWae kumain kana?")
            ...
            for reaction in giveawaymsg.reactions:
                if reaction.emoji == '🎉':
                    ...
                    if len(users) == 0:
                      await ctx.send("No winner was decided or lack of participants")
                    list_of_string = []
